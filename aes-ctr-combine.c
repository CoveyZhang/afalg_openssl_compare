...
int aes_ctr_openssl(char *input_buf, char *key_buf, char *iv_buf, char *openssl_buf)
{
	int i;
	int mlen = 0;
	int flen = 0;
	...
	EVP_EncryptInit_ex(ctx, EVP_aes_128_ctr(), NULL, key_buf, iv_buf);
...
int aes_ctr_test()
{
	...
	for (int i = 0; i < 100; ++i)
	{
		...
		aes_ctr_openssl(input_buf, key_buf, iv_buf, openssl_buf);
		ret = array_equal(alafg_buf, openssl_buf, 64);
		if (ret < 0)
		{
			printf("AES-CTR: AFALG AND OPENSSL RESULTS DIFFER\n");
			return -1;
		}
	}
