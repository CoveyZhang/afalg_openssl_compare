...
int aes_ecb_afalg(char *input_buf, char *key_buf, char *alafg_buf)
{
	int i;
	int opfd;
	int tfmfd;
...
int aes_ecb_openssl(char *input_buf, char *key_buf, char *openssl_buf)
{
	int i;
	int mlen = 0;
	int flen = 0;
	...
	EVP_EncryptInit_ex(ctx, EVP_aes_128_ecb(), NULL, key_buf, NULL);
...
int aes_ecb_test()
{
	...
	for (int i = 0; i < 100; ++i)
	{
		...
		aes_ecb_afalg(input_buf, key_buf, alafg_buf);
		aes_ecb_openssl(input_buf, key_buf, openssl_buf);
		ret = array_equal(alafg_buf, openssl_buf, 64);
		if (ret < 0)
		{
			printf("AES-ECB ERROR: AFALG AND OPENSSL RESULTS DIFFER\n");
			return -1;
		}
	}
