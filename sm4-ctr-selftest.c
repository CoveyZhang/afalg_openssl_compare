...
int sm4_ctr_afalg_de(char *input_buf, char *key_buf, char *iv_buf, char *alafg_buf)
{
	int i;
	int opfd;
	int tfmfd;
...
int sm4_ctr_test()
{
	...
	for (int i = 0; i < 100; ++i)
	{
		...
		sm4_ctr_afalg_de(alafg_buf, key_buf, iv_buf, de_buf);
		ret = array_equal(input_buf, de_buf, 64);
		if (ret < 0)
		{
			printf("SM4-CTR ERROR: AFALG AND OPENSSL RESULTS DIFFER\n");
			return -1;
		}
	}
